...
> At the end of this, you will be able to describe what an Action is and
> some basic uses of it, as well as being able to create a simple
> Action.
...
-   GitHub Actions are YAML files that must be placed in the
    `.github/workflows/` folder in a repository.
...
lesson in your web browser, except that a server is a connected set of
...
computations, and most often are used to run tasks for websites. All of
...
to GitHub to know about and use this file, it has to be kept in the
...
to only run the Action when an Issue (like bug reports, questions to
ask, or tasks to do) is created (called "opened"), you write:
...
::: callout-tip
The way that commands are written as a shortened uses the `.` to
separate each level. So for the one above, it would be `on.issues.type`.
:::
...
merge the change into the repository) or for `push` (any time a change
is uploaded to the repository). If you wanted to trigger on both, it
would look like:
...
modify, *and* save a file. For security purposes, you want to limit an
...
apply to all `jobs`. If you only wanted one job to have a specific
permission, you can use `permissions` as a nested level item in the
`jobs` task (more on that below).

#### `jobs` section

This section is composed of its own set of subsections and is where the
actual instructions are listed that form the core of an Action. You can
write as many jobs underneath this section as you need in order to
complete the task you want the GitHub servers to complete.

Each job is composed of a few sections:
...
-   The `run` section, which is the actual code to run. If you use the
    ...
    code that you can use in the `run` section is Bash code.

An example of one job with one step might look like:
...
Pages](https://pages.github.com/) so it can be hosted online and be
...
website. At least for this learning short, we wouldn't get into any
details on using Quarto, we'll only add the GitHub Action to the
repository that will build a website for you using Quarto.
...
else pushes changes to the `main` branch of the repository. So we would
start with:
...
Because we want to GitHub to host the website on their GitHub Pages
...
So we'll first set up the `jobs` section with the `name` and to
...
Note the use of `@4` to indicate the version of the `checkout` action to
use. Next, we'll install Quarto:
...
In this case, we need to provide the Quarto `setup` action (using
...
the Markdown files within the repository can build them into a website!

## Summary

-   You can use GitHub's servers to run code or a series of tasks by
    through their GitHub Actions feature.
-   An Action is a `.yml` file that is found in the `.github/workflows/`
    folder of a Git repository.
-   One Action fil contains a list of instructions for the server to run
...
-   There are three main sections to an Action: `on`, `permissions`, and
    ...
    -   The optional `permissions` section tells GitHub what extra
        permissions the Action can have, if needed.
...
-   There are a large number of pre-made Actions available from GitHub
    and other sources that you can use in your own Actions.
